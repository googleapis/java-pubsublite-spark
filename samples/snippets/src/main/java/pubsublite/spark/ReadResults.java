...
public class ReadResults {
    ...
    public static void main(String[] args) {
        ...
        Set<String> missingVars =
                Sets.difference(
                        ImmutableSet.of(REGION, ZONE_ID, SUBSCRIPTION_ID_RESULT, PROJECT_NUMBER),
                        env.keySet());
        ...
        subscriberExample(cloudRegion, zoneId, projectNumber, subscriptionIdResult).forEach(
                (m) ->
                    System.out.println(m.getData().toStringUtf8().replace("_", ": "))
        );
